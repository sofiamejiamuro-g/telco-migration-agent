...
def before_model_callback(callback_context: CallbackContext, llm_request: LlmRequest) -> Optional[LlmResponse]:
    if callback_context and hasattr(callback_context, "variables") and callback_context.variables is not None:
        ...
        # Check if user input contains digits (PIN or OTP) or confirmation
        if llm_request and llm_request.contents and llm_request.contents[-1].parts:
            for p in llm_request.contents[-1].parts:
                if getattr(p, "text", None):
                    t = p.text.strip()
                    # 4-digit PIN, 6-digit OTP, or numeric digits
                    if re.search(r"\b\d{4,6}\b", t) or "yes" in t.lower() or "oui" in t.lower() or "pin" in t.lower() or "code" in t.lower():
                        callback_context.variables["auth_status"] = "VERIFIED"
                        callback_context.variables["identification_status"] = "VERIFIED"
